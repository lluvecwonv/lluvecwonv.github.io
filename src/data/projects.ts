...
<tr><td>SPT Agent</td><td>관점 전환 유도</td><td>Cross-topic</td><td>SPT</td><td>Social Perspective Taking</td></tr>
  ...
  },
  {
    ...
    description:
      '기후변화로 인한 국가 유산 훼손을 탐지하기 위해 DETR 기반 다중 객체 탐지 모델을 개발하고, Flutter 기반 현장 조사 앱과 연동한 End-to-End 시스템.',
    ...
    sections: [
      {
        heading: '프로젝트 개요',
        body: '기후변화로 인한 국가 유산 훼손을 탐지하기 위해 DETR 기반 다중 객체 탐지 모델을 개발하고, Flutter 기반 현장 조사 앱과 연동하였습니다.',
      },
      {
        heading: '데이터 구축 및 품질 관리',
        body: '초기에는 데이터가 분산되어 있고 레이블 기준이 명확하지 않아, 팀원 간 동일한 손상이라도 다르게 라벨링되는 문제가 있었습니다. 이로 인해 클래스별 성능 편차가 크게 발생하였습니다. 이를 해결하기 위해 Label Studio를 직접 구축하여 레이블링 작업을 중앙에서 관리하고, 팀원들이 동일한 환경에서 작업할 수 있도록 정비했습니다. 또한 레이블링 과정에서 발생하는 기준 불일치를 줄이기 위해 교차 검수 방식으로 피드백을 주고받으며 클래스 정의를 지속적으로 정교화했습니다. 데이터 품질을 안정화한 이후에는, 부후·충해 등 데이터가 부족한 클래스에 대해 회전, 크롭, 노이즈 추가 등의 augmentation을 적용하여 데이터 불균형 문제를 완화했습니다.',
      },
      {
        heading: 'AI 손상 자동 탐지',
        body: 'DETR(Detection Transformer) 모델을 활용하여 8개의 손상 클래스(갈라짐, 균열, 압괴/터짐, 부후, 박락, 탈락, 충해 등)에 대한 다중 객체 탐지를 수행하였습니다. 고품질 데이터셋 구축과 학습 파라미터 최적화를 통해 클래스 간 성능 편차를 줄이며 mAP(Mean Average Precision) 0.615를 달성하였습니다.',
        images: [
