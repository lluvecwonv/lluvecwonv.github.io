...
학습 알고리즘 L이 데이터셋 X를 학습된 모델 Θ̂에 매핑한다고 하자. X에 대한 정보가 Θ̂에 얼마나 저장되었는지는 상호 정보량(mutual information)으로 측정할 수 있다:

```
mem(X, Θ̂) = I(X, Θ̂) = H(X) - H(X|Θ̂)
```

여기서 일반화를 분리하기 위해, 사전 모델(prior) Θ를 조건으로 하여 **unintended memorization**을 정의한다:
...
**Proposition (Super-additivity of Unintended Memorization)**: X = (X₁, ..., Xₙ)이 i.i.d. 데이터셋일 때:
...
```
mem^K(x̂, x) = I^K(θ̂, x) = H^K(x) - H^K(x|θ̂)
```
...
**Proposition (Kolmogorov ≈ Shannon)**: i.i.d. 데이터셋 분포에서, Kolmogorov memorization의 기대값은 Shannon memorization과 상수 ε 이내로 근사한다.

### 2.3 실제 추정 방법

Kolmogorov complexity는 계산 불가능하므로, **arithmetic coding**을 이용해 근사한다:

- **H^K(x|θ̂)**: 학습된 모델의 negative log-likelihood로 추정, 즉 -log(p(x|θ̂))
- **H^K(x|θ̂, θ)**: 두 모델 중 더 나은 압축률을 사용, 즉 -log(max{p(x|θ̂), p(x|θ)})
...
모델 용량에 도달하면, mem(X, L(X))은 데이터셋 크기가 증가해도 더 이상 증가하지 않는다.
...
비선형 최소자승법으로 최적값을 구한다: **c₁ = 1.34, c₂ = -0.034, c₃ = -33.14**.
