...
학습 데이터로는 오픈소스 **Pile** (Gao et al., 2020) 코퍼스를 사용한다. Pile은 공개 데이터셋으로, 총 **146,432,000개의 행(row)** 으로 구성되어 있으며, 각 행은 **2,048 토큰** 길이의 청크(chunk)로 이루어져 있다. 전체 데이터 크기는 약 **800GB** 이다.

기억화 점수를 측정하는 실험 과정은 다음과 같다. 샘플링 없이 **Pile 데이터 전체를 순회(iterate)** 한다. 구체적으로, context 크기가 32, continuation 크기가 96인 경우를 예로 들면:

1. 각 행의 **앞 32개 토큰(context)** 을 모델에 입력으로 제공한다.
2. Pythia 모델이 이어지는 **96개 토큰(continuation)** 을 greedy decoding으로 생성한다.
3. 생성된 토큰 ID와 실제 데이터의 정답(gold) 토큰 ID를 비교하여 해당 행의 **기억화 점수(memorization score)** 를 계산한다.

이 과정을 Pile의 **전체 146M개 행에 대해 반복** 하며, 여러 CUDA 디바이스에 분산하여 병렬 처리한다.
