...
**방법:** 컨텍스트 토큰 시퀀스 $C = \{c_0 \cdots c_n\}$을 LLM에 프롬프트로 제공하고, greedy decoding으로 continuation 토큰을 생성한다. 예측된 continuation과 실제 continuation을 비교하여 기억화 점수를 계산한다:

$$M(X, Y) = \frac{\sum_{i=0}^{n} x_i == y_i}{L(Y)}$$

- $X = \{x_0 \cdots x_n\}$: 예측된 continuation 토큰 시퀀스
- $Y = \{y_0 \cdots y_n\}$: 실제 continuation 토큰
- $M(X, Y) = 1$이면 **완전 기억화(K-extractable)**
- $M(X, Y) = 0$이면 **비기억화(unmemorized)**
...
**Pythia 모델** (Biderman et al., 2023)을 사용한다. Pythia는 다양한 크기의 LLM을 동일한 학습 순서와 오픈소스 **Pile** 코퍼스로 학습하여 실험의 안정성을 보장한다.

- **모델 크기:** 70M, 160M, 410M, 1B, 2.8B, 6.9B, 12B
- **데이터:** 중복 제거된(deduplicated) 사전학습 데이터 버전 사용 (중복 문장의 기억화 확률이 기하급수적으로 증가하므로)
