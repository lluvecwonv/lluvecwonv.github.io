...
기대값은 동일한 비율의 near-duplicate/exact copy로 학습된 모든 모델에 대해 취한다. Zhang et al. (2023)과 동일하게, 이 모델들이 균등 분포(uniformly distributed)라고 가정하면, 기대값은 **여러 학습 모델에 대한 log(P(s | p))의 단순 평균**으로 축소된다.

**PA Memorization (Equation 7):**

Counterfactual memorization과의 비교를 위해, PA memorization도 원래의 비율 형태 E_M[P(s|p;M)/v̂_s] 대신 **log 형태**로 측정한다:

> E_M[log(P(s | p; M) / v̂_s)] = E_M[log(P(s | p; M))] − E_M[log(v̂_{s;M})]

이 분해가 핵심이다: 좌변의 PA memorization은 **target model M에서의 조건부 확률(첫 번째 항)**과 **suffix의 통계적 commonality(두 번째 항)**의 차이로 표현된다. 즉, P(s|p)가 높더라도 v̂_s(≈ P(s))도 높으면 PA memorization 값은 낮아진다 — 이것이 일반화와 암기를 구분하는 메커니즘이다.
