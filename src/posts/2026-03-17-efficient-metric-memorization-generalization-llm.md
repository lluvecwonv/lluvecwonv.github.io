...
PA memorization이 실제로 counterfactual memorization과 유사한 정보를 포착하는지 검증하기 위한 통제된 실험이다. 핵심 아이디어는 학습 데이터에서 exact copy와 near-duplicate의 비율을 체계적으로 변화시켜, 모델이 "암기"와 "일반화" 중 어느 쪽에 의존하는지를 조절한 뒤, 두 메트릭이 일관된 패턴을 보이는지 확인하는 것이다.

### 4.1 모델 학습 (Model Training)
...
기존 연구(Liu et al., 2025; Zhang et al., 2023)에서는 높은 오버랩(예: 50% 이상)의 duplicate을 사용했으나, 본 논문은 **20% 토큰 오버랩**이라는 보수적인 기준을 채택한다. 이유는:

- Lee et al. (2022)에 따르면, 높은 오버랩 시퀀스를 제거하면 모델 성능이 향상됨
- 현대 학습 파이프라인(Touvron et al., 2023; Zhang et al., 2022)에서는 고오버랩 데이터가 전처리 단계에서 제거됨
- 따라서 20%의 낮은 오버랩으로도 일반화가 가능함을 보이는 것이 더 현실적
...
학습 데이터에 타겟 시퀀스의 **exact copy와 near-duplicate의 비율을 체계적으로 변화**시켜 7가지 데이터셋을 구성한다:

| 설정 | (Exact Copies, Near-Duplicates) | 의미 |
|------|-------------------------------|------|
| 1 | (0, 180) | 순수 일반화 — exact copy 없이 near-dup만으로 학습 |
| 2 | (10, 150) | 약한 암기 신호 |
| 3 | (20, 120) | |
| 4 | (30, 90) | 중간 |
| 5 | (40, 60) | |
| 6 | (50, 30) | 강한 암기 신호 |
| 7 | (60, 0) | 순수 암기 — near-dup 없이 exact copy만으로 학습 |

**핵심 설계 원칙:**

- **총 학습 셋 크기는 항상 1,000 시퀀스로 고정** — exact copy와 near-duplicate의 구성비만 변경하여 다른 변수를 통제
- 예: 설정 2에서는 기존 180개의 near-duplicate 중 30개를 10개의 exact copy로 대체 → 150 near-dup + 10 exact
- Exact copy가 많을수록 모델은 **counterfactual memorization**을 더 많이 보일 것으로 예상 (verbatim으로 학습했기 때문)
- Near-duplicate이 많을수록 모델은 **일반화**에 의존 — 비슷하지만 동일하지 않은 데이터로부터 패턴을 학습

25개 타겟 시퀀스 × 7가지 설정 = **175개 target 모델** 학습. 각 설정에서 서로 다른 seed로 여러 번 반복하여 총 350개 이상의 모델 학습.

#### Baseline Model 학습 전략

Counterfactual memorization 계산에는 target model과 baseline model의 비교가 필요하다:
...
이 설계의 의도는: exact target 데이터가 제거되더라도, 일반화할 수 있는 유사 데이터(near-duplicate)가 학습 데이터에 여전히 존재하는 현실적 시나리오를 시뮬레이션하는 것이다.

### 4.2 측정 방법

x = p‖s (prefix와 suffix의 결합)로 해석하며, target model M과 baseline model M'를 사용한다. 모델 정확도는 extractable memorization 정의(Carlini et al., 2022)를 따라 log(P(s | p; M))으로 측정한다.

**Counterfactual Memorization (Equation 6):**
> E_M[log(P(s | p; M))] − E_{M'}[log(P(s | p; M'))]

동일한 비율의 near-duplicate/exact copy로 학습된 모든 모델에 대해 기대값을 취한다. 모델들이 균등 분포라고 가정하면, 이 기대값은 여러 학습 모델에 대한 log(P(s | p))의 평균으로 축소된다.

**PA Memorization (Equation 7):**
> E_M[log(P(s | p; M) / v̂_s)] = E_M[log(P(s | p; M))] − E_M[log(v̂_{s;M})]

PA memorization은 **target model M만** 필요하고 baseline model M'가 불필요하다. 이것이 PA memorization의 핵심 장점이다.
...
- Near-duplicate이 줄어들면서 시퀀스가 덜 generic해질수록, 두 메트릭 모두 "이것은 암기다"라는 판단을 강화
- 이는 PA memorization이 추가 모델 학습 없이도 counterfactual memorization과 유사한 정보를 포착할 수 있음을 시사
