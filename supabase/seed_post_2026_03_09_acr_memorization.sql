-- Seed: ACR Memorization 연구노트 (updated with experimental setup)
-- Run in Supabase SQL Editor

INSERT INTO posts (slug, title, date, summary, tags, category, content, published)
VALUES (
...
**Llama-2-7B-chat** (Touvron et al., 2023): 7B 파라미터, instruction-tuned 모델. In-Context Unlearning(ICUL) 실험(Section 5.1)과 Harry Potter unlearning 검증(Section 5.3)에 사용된다. Chat 모델이므로 instruction 태그(`[INST]`, `[/INST]`) 사이에 최적화 토큰을 배치하는 변형 전략을 사용한다.
...
**Associated Press (2023년 11월)**: 모델 학습 시점 이후에 발행된 뉴스 기사. 학습 데이터에 포함되지 않으므로 memorized로 판정되면 안 되는 negative control.

**Random Sequences**: 토큰 vocabulary에서 균일 무작위로 샘플링한 100개의 시퀀스 (길이 3~17 토큰). 의미 없는 gibberish로, 절대 압축 불가능해야 하는 또 다른 negative control.
